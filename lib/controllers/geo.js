...
export const getFeatures = ({ initPolygons, polygonFeatureTypes, autoLabelLines, hiddenAmenities, useTimerangeData, filter, featuresMaxBounds, localSources, }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (initPolygons) {
        ...
        polygonFeatureTypes.forEach((featureType, index) => {
            const shopPolygons = res.data.features.filter((f) => f.properties.type === featureType && f.geometry.type === 'MultiPolygon');
                ...
                if (feature.properties.type === 'poi') {
                    ...
                    if (!connectedPolygon && feature.properties.metadata && feature.properties.metadata.polygon_id) {
                        connectedPolygon = res.data.features.find((f) => {
                            var _a, _b;
                            return ((_a = f.properties.id) === null || _a === void 0 ? void 0 : _a.replace(/\{|\}/g, '')) ===
                    ...
                    if (connectedPolygon) {
                        ...
                        feature.properties._dynamic.polygon_id = (_a = connectedPolygon.properties.id) === null || _a === void 0 ? void 0 : _a.replace(/\{|\}/g, '');
                        connectedPolygon.properties._dynamic = connectedPolygon.properties._dynamic
                            ? connectedPolygon.properties._dynamic
                            : {};
                        connectedPolygon.properties._dynamic.id = (_b = connectedPolygon.properties.id) === null || _b === void 0 ? void 0 : _b.replace(/\{|\}/g, '');
...
export const getFeaturesBundle = ({ initPolygons, polygonFeatureTypes, autoLabelLines, hiddenAmenities, useTimerangeData, filter, bundleUrl, }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (initPolygons) {
        ...
        const shopPolygons = data.features.filter((f) => f.geometry.type === 'MultiPolygon' && polygonFeatureTypes.includes(f.properties.type));
        ...
        data.features
            ...
            feature.properties.id = feature.properties.id ? feature.properties.id.replace(/\{|\}/g, '') : null;
            // check if feature is inside a polygon
            let connectedPolygon = shopPolygons.find((polygon) => {
                var _a, _b;
                if (((_a = polygon.properties) === null || _a === void 0 ? void 0 : _a.level) === ((_b = feature.properties) === null || _b === void 0 ? void 0 : _b.level) && booleanPointInPolygon(feature, polygon)) {
                    return polygon;
                }
            });
            // or if not check if the polygon is defined in metadata and use it instead
            if (!connectedPolygon && ((_a = feature.properties.metadata) === null || _a === void 0 ? void 0 : _a.polygon_id)) {
                const polygonId = feature.properties.metadata.polygon_id.replace(/\{|\}/g, '');
                connectedPolygon = data.features.find((f) => { var _a; return ((_a = f.properties.id) === null || _a === void 0 ? void 0 : _a.replace(/\{|\}/g, '')) === polygonId; });
            }
            if (connectedPolygon) {
                feature.properties._dynamic = feature.properties._dynamic ? feature.properties._dynamic : {};
                feature.properties._dynamic.id = feature.id;
                feature.properties._dynamic.type = 'poi-custom';
                feature.properties._dynamic.amenity = feature.properties.amenity;
                feature.properties._dynamic.polygon_id = (_b = connectedPolygon.properties.id) === null || _b === void 0 ? void 0 : _b.replace(/\{|\}/g, '');
                connectedPolygon.properties._dynamic = connectedPolygon.properties._dynamic
                    ? connectedPolygon.properties._dynamic
                    : {};
                connectedPolygon.properties._dynamic.id = (_c = connectedPolygon.properties.id) === null || _c === void 0 ? void 0 : _c.replace(/\{|\}/g, '');
                connectedPolygon.properties._dynamic.type = `${polygonFeatureTypes.find((type) => type === connectedPolygon.properties.type)}-custom`;
                connectedPolygon.properties._dynamic.poi_id = feature.properties.id;
                connectedPolygon.properties._dynamic.amenity = feature.properties.amenity;
                // id have to be changed to numeric type so feature state will work
                connectedPolygon.id = removeNonNumeric(connectedPolygon.id);
                ...
                if (connectedLabelLine) {
                    ...
                    connectedLabelLine.properties.id = connectedLabelLine.id;
                    connectedLabelLine.properties._dynamic.id = connectedLabelLine.id;
                    connectedLabelLine.properties.type = `${polygonFeatureTypes.find((type) => type === connectedPolygon.properties.type)}-label`;
                    connectedLabelLine.properties._dynamic.type = `${polygonFeatureTypes.find((type) => type === connectedPolygon.properties.type)}-label`;
                    connectedLabelLine.properties._dynamic.poi_id = feature.properties.id;
                    connectedLabelLine.properties._dynamic.amenity = feature.properties.amenity;
                    connectedLabelLine.properties._dynamic.polygon_id = connectedPolygon.properties._dynamic.id;
                    connectedLabelLine.properties._dynamic.length = Math.ceil(length(connectedLabelLine) * 1000);
                ...
                if (!connectedLabelLine) {
                    ...
                    if (labelLine && labelLine !== undefined && labelLine.length > 0) {
                        ...
                        if (parsedLabelLine[0] instanceof Array && parsedLabelLine[1] instanceof Array) {
                            ...
                            labelLineFeature.properties.id = `${removeNonNumeric(connectedPolygon.id)}9999`;
                            labelLineFeature.id = `${removeNonNumeric(connectedPolygon.id)}9999`;
                            labelLineFeature.properties.type = `${polygonFeatureTypes.find((type) => type === connectedPolygon.properties.type)}-label`;
                            labelLineFeature.properties._dynamic.type = `${polygonFeatureTypes.find((type) => type === connectedPolygon.properties.type)}-label`;
                            labelLineFeature.properties._dynamic.length = Math.ceil(length(labelLineFeature) * 1000);
                            connectedPolygon.properties._dynamic.label_id = labelLineFeature.properties.id;
                            featuresToAdd.push(labelLineFeature);
                        }
                        ...
                        labelBorder.properties.id = `${removeNonNumeric(connectedPolygon.id)}9999`;
                        labelBorder.id = `${removeNonNumeric(connectedPolygon.id)}9999`;
                        labelBorder.properties.type = `${polygonFeatureTypes.find((type) => type === connectedPolygon.properties.type)}-label`;
                        labelBorder.properties._dynamic.type = `${polygonFeatureTypes.find((type) => type === connectedPolygon.properties.type)}-label`;
                        connectedPolygon.properties._dynamic.label_id = labelBorder.properties.id;
                        featuresToAdd.push(labelBorder);
                    }
                }
