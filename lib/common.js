...
const optimizeFeature = (feature) => {
    ...
    const optimizedProperties = Object.assign({}, feature.properties);
...
const shortenCoordinates = ({ coords, decimals }) => {
    if (Array.isArray(coords)) {
        return coords.map((c) => Array.isArray(c) ? shortenCoordinates({ coords: c, decimals }) : Number(c.toFixed(decimals)));
    }
    return Number(coords.toFixed(decimals));
};
const isElevator = (poi) => {
    return poi.properties.type === POI_TYPE.ELEVATOR;
};
const isEscalator = (poi) => {
    return poi.properties.type === POI_TYPE.ESCALATOR;
};
const isStairCase = (poi) => {
