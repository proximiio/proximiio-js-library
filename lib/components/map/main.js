...
export class Map {
    ...
    fetch() {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const places = yield getPlaces().catch((error) => this.handleControllerError(error));
            const style = yield getStyle().catch((error) => this.handleControllerError(error));
            const styles = yield getStyles().catch((error) => this.handleControllerError(error));
            ...
            const kiosks = yield getKiosks().catch((error) => this.handleControllerError(error));
            if (places && features && floors && style && kiosks) {
                ...
                this.state = Object.assign(Object.assign({}, this.state), { initializing: false, place, places: places.data, floor, floors: floors.data, kiosks: kiosks.data, style,
                    styles,
                    amenities,
                    features, allFeatures: new FeatureCollection(features), levelChangers: new FeatureCollection({ features: levelChangers }), latitude: centerVar[1], longitude: centerVar[0], zoom: this.defaultOptions.zoomLevel ? this.defaultOptions.zoomLevel : (_c = this.defaultOptions.mapboxOptions) === null || _c === void 0 ? void 0 : _c.zoom, noPlaces: places.data.length === 0, user });
        ...
        if (this.map) {
            ...
            if (this.defaultOptions.kioskSettings) {
                ...
                if (this.defaultOptions.kioskSettings.showLabel) {
                    kioskLayer = Object.assign(Object.assign({}, kioskLayer), { layout: Object.assign(Object.assign({}, kioskLayer.layout), { 'text-field': translations[this.defaultOptions.language].YOU_ARE_HERE, 'text-font': ['Quicksand Bold', 'Noto Sans Arabic Bold'], 'text-offset': [0, 0.6], 'text-anchor': 'top', 'text-allow-overlap': true, 'text-size': 18 }), paint: {
                            'text-halo-width': 1,
        ...
        if (this.map) {
            ...
            this.state.style.addSource('raster-tiles', {
                ...
                tiles: ((_a = this.defaultOptions.rasterTilesOptions) === null || _a === void 0 ? void 0 : _a.tilesUrl)
                    ? [
                        `${((_b = this.defaultOptions.rasterTilesOptions) === null || _b === void 0 ? void 0 : _b.useProxy) ? 'https://api.proximi.fi/imageproxy/source=' : ''}${this.defaultOptions.rasterTilesOptions.tilesUrl}`,
                    ]
                    : [
                        `${((_c = this.defaultOptions.rasterTilesOptions) === null || _c === void 0 ? void 0 : _c.useProxy) ? 'https://api.proximi.fi/imageproxy/source=' : ''}${metadata['proximiio:raster:tileurl']}`,
                    ],
                tileSize: ((_d = this.defaultOptions.rasterTilesOptions) === null || _d === void 0 ? void 0 : _d.tileSize)
                    ...
                    : metadata['proximiio:raster:tilesize']
                        ? metadata['proximiio:raster:tilesize']
                        : 256,
                attribution: ((_e = this.defaultOptions.rasterTilesOptions) === null || _e === void 0 ? void 0 : _e.attribution)
            ...
            this.state.style.addLayer({
                ...
                type: 'raster',
                source: 'raster-tiles',
                minzoom: ((_f = this.defaultOptions.rasterTilesOptions) === null || _f === void 0 ? void 0 : _f.minZoom)
                    ? this.defaultOptions.rasterTilesOptions.minZoom
                    : metadata['proximiio:raster:minzoom']
                        ? metadata['proximiio:raster:minzoom']
                        : 15,
                maxzoom: ((_g = this.defaultOptions.rasterTilesOptions) === null || _g === void 0 ? void 0 : _g.maxZoom)
                    ? this.defaultOptions.rasterTilesOptions.maxZoom
                    : metadata['proximiio:raster:maxzoom']
                        ? metadata['proximiio:raster:maxzoom']
                        : 22,
            }, ((_h = this.defaultOptions.rasterTilesOptions) === null || _h === void 0 ? void 0 : _h.beforeLayer)
    ...
    handlePolygonSelection(poi) {
        ...
        if (connectedPolygonId) {
            this.selectedPolygon = this.state.allFeatures.features.find((i) => {
                var _a, _b;
                return ((_a = i.properties._dynamic) === null || _a === void 0 ? void 0 : _a.id) === connectedPolygonId &&
                    this.defaultOptions.polygonLayers
                        .map((layer) => `${layer.featureType}-custom`)
                        .includes((_b = i.properties._dynamic) === null || _b === void 0 ? void 0 : _b.type);
            });
            if (this.selectedPolygon) {
                this.map.setFeatureState({
                    source: 'main',
                ...
                for (const f of activeFeatures) {
                    const polygon = this.state.allFeatures.features.find((i) => {
                        ...
                        return ((_a = i.properties._dynamic) === null || _a === void 0 ? void 0 : _a.id) === ((_b = f.properties._dynamic) === null || _b === void 0 ? void 0 : _b.polygon_id) &&
                            this.defaultOptions.polygonLayers
                                .map((layer) => `${layer.featureType}-custom`)
                                .includes((_c = i.properties._dynamic) === null || _c === void 0 ? void 0 : _c.type);
                    });
                    if (polygon) {
                        this.map.setFeatureState({
                            source: 'main',
                ...
                for (const GLTFModel of self.defaultOptions.GLTFModels) {
                    ...
                    for (const feature of features) {
                        ...
                        loader.load(GLTFModel.modelUrl, (gltf) => {
                            ...
                            const { dEastMeter: modelEast, dNorthMeter: modelNorth } = self.calculateDistanceMercatorToMeters(sceneOriginMercator, modelMercator);
                            model.position.set(modelEast, modelAltitude, modelNorth);
                            if (GLTFModel.scale)
                                model.scale.set(GLTFModel.scale, GLTFModel.scale, GLTFModel.scale);
                            if (GLTFModel.rotateX)
                                model.rotation.x = THREE.MathUtils.degToRad(GLTFModel.rotateX);
                            if (GLTFModel.rotateY)
                                model.rotation.y = THREE.MathUtils.degToRad(GLTFModel.rotateY);
                            if (GLTFModel.rotateZ)
                                model.rotation.z = THREE.MathUtils.degToRad(GLTFModel.rotateZ);
                            if (GLTFModel.translateX)
                                model.translateX(GLTFModel.translateX);
                            if (GLTFModel.translateY)
                                model.translateY(GLTFModel.translateY);
                            if (GLTFModel.translateZ)
                                model.translateZ(GLTFModel.translateZ);
                            scene.add(model);
                        });
                    }
                }
    ...
    calculateDistanceMercatorToMeters(from, to) {
    ...
    onPlaceSelect(place, zoomIntoPlace, floorLevel) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const floors = yield getPlaceFloors(place.id).catch(this.handleControllerError);
            if (floors) {
    ...
    updateImages() {
        ...
        this.state.features.features
            ...
            .forEach((f) => {
            this.map.loadImage(`${f.properties.metadata['anchor-logo']}?token=${this.state.user.token}`, (error, image) => {
                if (error)
                    throw error;
