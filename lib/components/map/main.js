...
import { chevron, pulsingDot, person as personIcon, floorchangeUpImage, floorchangeDownImage, popupImage, routeStartSvg, routeFinishSvg, createHeadingArrow, } from './icons';
...
export class Map {
  ...
  `;
    ...
    onFloorSelect(floor) {
        ...
        if (map) {
            ...
            if (map.getLayer('heading-arrow-layer')) {
                const filter = ['all', ['==', ['to-number', ['get', 'level']], floor.level]];
                map.setFilter('heading-arrow-layer', filter);
    ...
    onSetCustomPosition({ coordinates, level, recenter = true, iconSize = 1.5, arrowSize = 1.25, }) {
        ...
        const shouldSwitchFloor = mostFrequentLevel !== this.state.floor.level &&
    ...
    animateCustomPosition(from, to, level, userBearing) {
