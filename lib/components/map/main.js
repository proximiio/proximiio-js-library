...
export class Map {
  ...
  `;
     ...
     *  });
     */
    findCombinedRoute({ start, connectingPoint, destination, autoStart = true, }) {
        const startFeature = typeof start === 'string' || start instanceof String
            ? this.state.allFeatures.features.find((f) => f.id === start || f.properties.id === start)
            : feature({ type: 'Point', coordinates: [start.lng, start.lat] }, { level: 0 });
        const connectingPointFeature = feature({ type: 'Point', coordinates: [connectingPoint.lng, connectingPoint.lat] }, { level: 0 });
        const destinationFeature = typeof destination === 'string' || destination instanceof String
            ...
            : feature({ type: 'Point', coordinates: [destination.lng, destination.lat] }, { level: 0 });
    ...
    setNavStep(step) {
        ...
        if (this.routingSource && this.routingSource.route && this.routingSource.route[`path-part-${newStep}`]) {
            ...
            if (route.properties.source !== (prevRoute === null || prevRoute === void 0 ? void 0 : prevRoute.properties.source) ||
