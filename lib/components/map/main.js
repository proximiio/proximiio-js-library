    ...
    function Map(options) {
        ...
        this.animateRoute = function (route) {
            if (_this.defaultOptions.routeAnimation.type === 'point') {
                ...
                var vizDuration_1 = _this.defaultOptions.routeAnimation.duration
                    ? _this.defaultOptions.routeAnimation.duration
                    : walkingDuration * (1 / multiplier);
                var fps = _this.defaultOptions.routeAnimation.fps;
                var frames_1 = Math.round(fps * vizDuration_1);
                console.log("Route Duration is ".concat(walkingDuration, " seconds"));
                console.log("Vizualization Duration is ".concat(vizDuration_1, " seconds"));
                console.log("Total Frames at ".concat(fps, "fps is ").concat(frames_1));
                // divide length and duration by number of frames
                var routeLength = turf.length(route);
                var incrementLength_1 = routeLength / frames_1;
                var interval = (vizDuration_1 / frames_1) * 1000;
                ...
                var animate_1 = function (timestamp) {
                    ...
                    if (progress < vizDuration_1 * 1000) {
                        var frameProgress = progress / (vizDuration_1 * 1000);
                        counter_1 = Math.floor(frameProgress * frames_1);
            ...
            if (_this.defaultOptions.routeAnimation.type === 'dash') {
                ...
                //@ts-ignore
                _this.map.getSource('lineAlong').setData(route);
        ...
        this.updateData = function (route, incrementLength, counter, frames) {
            ...
            if (counter === frames) {
                ...
                //@ts-ignore
                _this.map.getSource('pointAlong').setData({
                    type: 'FeatureCollection',
                    features: [],
                });
                if (_this.defaultOptions.routeAnimation.looping) {
                    setTimeout(function () {
                        _this.animateRoute(route);
                    }, 2000);
                }
            }
        };
