  ...
  _createClass(Wayfinding, [{
    ...
    value: function rebuildData() {
      ...
      this.corridors.forEach(function (corridor, _) {
        ...
        for (var _i = 0; _i < coordinateList.length - 1; _i++) {
          ...
          var isHill = corridor.properties.ramp === true; // Mark points as Hill if corridor is Hill
    ...
    value: function _checkTimeBased(feature) {
      ...
      if (feature.properties.workingHours !== undefined) {
        ...
        var openings = feature.properties.workingHours[currentDay]; // Current hour in UTC

        var currentHour = parseInt(now.getUTCHours());
        var currentMinute = parseInt(now.getUTCMinutes());
        var current = currentHour * 60 + currentMinute; // if there's no data means it's always close
        ...
        var open = openingHour * 60 + openingMinute; // Retrieve closing

        var closing = openings[1].split(":");
        var closingHour = parseInt(closing[0]);
        var closingMinute = parseInt(closing[1]);
        var close = closingHour * 60 + closingMinute; // It's always open
