...
const compareResults = (pois: SortedPoiItemModel[], visionTextResults: string[]) => {
  ...
  pois.forEach((poi) => {
    ...
    titles.forEach((title) => {
      ...
      visionTextResults.forEach((ocrWord) => {
        const containsWordScore = containsWordFromTitle(ocrWord, title);
        // Calculate similarity score between store title and OCR word
        const similarityScore = calculateSimilarity(title, ocrWord);
